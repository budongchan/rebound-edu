...
import { Briefcase, Search, DollarSign, CheckCircle, TrendingUp } from "lucide-react";
...
export default function AdminCommissionsPage() {
  ...
  const activeCount = commissions.filter((c) => ["requested", "accepted", "in_progress"].includes(c.status)).length;
  const totalRevenue = commissions
    .filter((c) => c.status === "settled")
    .reduce((s, c) => s + c.final_amount, 0);
  const totalFee = commissions
    .filter((c) => c.status === "settled")
    .reduce((s, c) => s + c.platform_fee, 0);
  const totalPayout = commissions
    .filter((c) => c.status === "settled")
    .reduce((s, c) => s + c.expert_payout, 0);
  ...
  return (
    <>
      <h2 className="text-base font-bold mb-5">의뢰 현황</h2>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        <StatCard label="총 거래액" value={`₩${formatPrice(totalRevenue)}`} />
        <StatCard
          label="플랫폼 수수료 수익"
          value={`₩${formatPrice(totalFee)}`}
          accent="#22c55e"
          sub={totalRevenue > 0 ? `평균 ${Math.round(totalFee / totalRevenue * 100)}%` : undefined}
        />
        <StatCard label="전문가 지급 총액" value={`₩${formatPrice(totalPayout)}`} accent="#3b82f6" />
      </div>
              ...
              {filtered.map((c) => {
                ...
                return (
                  <tr key={c.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      {c.status === "settled" ? (
                        <span className="text-sm text-red-500">₩{formatPrice(c.platform_fee)}</span>
                      ) : (
                        <span className="text-sm text-gray-300">-</span>
                      )}
                    </td>
            ...
            <div className="space-y-3 mb-4">
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-700">{settlingCommission.expert?.name || "-"}</span>
                </div>
                <div className="flex justify-between text-sm">
